...
import { Plus, Pencil, Trash2, Loader2, BookOpen, Save, Variable, ChevronDown, ChevronUp, Sparkles } from "lucide-react";
...
export default function MyClauses() {
  ...
  if (!access.isPro) {
    return <ProUpgradeScreen feature="custom_clauses" />;
  }
  return <MyClausesInner />;
}

function MyClausesInner() {
  ...
  const clausesQuery = useQuery({
    queryKey: ['my-clause-library', myProfile?.id],
    queryFn: async () => {
  ...
  const upsertMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['my-clause-library', myProfile.id] });
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        ...
        >
          <div>
            ...
            <p className="mt-1 text-zinc-400">
              Your private library of reusable clauses. They appear in the contract builder alongside the standard library — only you can see them.
            </p>
          </div>
          <Button
        ...
        {clausesQuery.isLoading ? (
          ...
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900/40 p-12 text-center">
            ...
            <h3 className="text-lg font-semibold mb-1">No custom clauses yet</h3>
            <p className="text-zinc-400 text-sm mb-5 max-w-md mx-auto">
              Create reusable clauses with your own language and variables — they'll be available when building any new contract.
            </p>
            <Button
              onClick={() => setEditing('new')}
              className="bg-violet-600 hover:bg-violet-700 gap-2 rounded-xl"
            >
              <Plus className="w-4 h-4" /> Create your first clause
            </Button>
          </div>
        ) : (
        ...
        <AlertDialogContent className="bg-zinc-900 border-zinc-800 text-white">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-zinc-400">
              <span className="block">"{confirmDelete?.title}" will be removed from your library.</span>
              <span className="block mt-2 text-amber-300">Already-signed contracts that used this clause keep the original text. Only future contracts lose access.</span>
...
function ClauseEditor({ clause, onClose, onSave, saving, error }) {
  ...
  const draftKey = isNew ? 'clause:my:new' : `clause:my:edit:${clause.id}`;
